\begin{figure}[H]
    ...
    \begin{tikzpicture}[initial text={...}, ->,>=stealth',shorten >=1pt,auto,node distance=1.8cm, semithick]
        ...
        \node[fill=gray!20, state]              (E) [above of=D] {$S_u$};
        \node[fill=gray!20, state]              (F) [below of=D] {$S_u$};
        ...
        \path   (A) edge node {} (B)
                ...
                (G) edge node {} (H);
    \end{tikzpicture}
    \caption{Surrounding state evaluated after and before the \textit{current} state}
    \label{fig:substates}
\end{figure}