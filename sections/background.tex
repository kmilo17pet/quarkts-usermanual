...
For more details, see the MIT License in \cite{mitlic}. 
...
Clear the specified parameter for the task. \index{\lstinline{qTask_Clear}}

\subsubsection*{Parameters:}
\begin{itemize}
    \item \lstinline{Task} : A pointer to the task node.
    \item \lstinline{param} : Use one of the following values:
    \begin{itemize}
        \item \lstinline{qTask_ClearIterations} : Clear the number of iterations.
        \item \lstinline{qTask_ClearTimeElapsed} : Clear the time elapsed.
        \item \lstinline{qTask_ClearCycles} : Clear the number of task activation's. 
        \item \lstinline{qTask_ClearSimpleNotifications} : Clear the notification value.
        \item \lstinline{qTask_ClearQueuedNotifications} : Clear all the queued notifications.
        \item \lstinline{qTask_ClearNotifications} : Clear all notifications (simple and queued).
    \end{itemize} 
\end{itemize}
