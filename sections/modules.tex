...
This workflow between the \textit{current} and the \textit{surrounding} state is shown in the graph below: 
\input{fig/substates.tex}

Figure \ref{fig:substates} shows that the \textit{surrounding state} (if enabled) are implicitly linked in the workflow to every state in the FSM. The \textit{surrounding state}, and the way they are called are described below:
...
Unlike normal states, a Surrounding state should not return anything, thus, the callback for the surrounding should be written as:
\medskip
...
If the Surrounding state its not defined at the moment of the FSM setup, they can be later installed using the \lstinline{qStateMachine_Attribute} function.
\medskip
...
\begin{itemize}
    ...
    \item \lstinline{LastReturnStatus} (read-only): The exit(or return) status of the last state. Can be also used in the \textit{Surrounding state} to perform the corresponding actions for every value, including the {qSM_BEFORE_ANY}.
