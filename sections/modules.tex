...
\subsubsection{Changing states and retrieving FSM data} \label{qsmhandlerfields} 
...
\begin{lstlisting}[style=CStyle]
qBool_t qStateMachine_SignalQueueSetup( qSM_t * const obj, qQueue_t *xQueue,
...
\begin{itemize}
    \item \lstinline{obj} : A pointer to the FSM object. 
    \item \lstinline{xQueue} : A pointer to the queue instance. 
...
\begin{tcolorbox}
\HandRight A FSM with a transition table performs better if it has a signal queue.
\end{tcolorbox}
