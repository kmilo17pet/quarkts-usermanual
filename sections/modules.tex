...
One important attribute of the \lstinline{qSM_State_t} object is the callback function, to describe the behavior specific to the state and a pointer to the parent state to define nesting of the state.
As shown in the figure, a state machine consist of a least one state the "top level" state.
...
\begin{itemize}
    ...
    \item \lstinline{childstart} : The first state to be executed (init-state). 
    \item \lstinline{Surrounding} : The surrounding callback function. To ignore pass \lstinline{NULL}.
    \item \lstinline{Data} : Represents the FSM arguments. To ignore pass \lstinline{NULL}. All arguments must be passed by reference and cast to (void *). Only one argument is allowed, so, for multiple arguments, create a structure that contains all of the arguments and pass a pointer to that structure. 
\end{itemize}  
...
It is a checkpoint before and after each state executes its activities through its callback. The behavior of this surrounding callback must be defined by the programmer.

\input{fig/surrounding}

When the surrounding callback is executed, it indicates its own checkpoint through the status attribute of the \lstinline{qSM_Handler_t} argument.
...
Using this API, the kernel will take care of the FSM by itself, \lstinline{qStateMachine_Run()} can be omitted. 
\medskip
...
The user can benefit from the event abstraction that is provided through signals. On the one hand, there would be a more uniform programming when writing states callbacks and on the other hand, the communication of the state machine from other contexts becomes easier.
...
\lstinline{qFalse} if there is a queue, and the signal cannot be inserted because it is full.
...
To enable this functionality in your state machine, the queue must be installed by using \lstinline{qStateMachine_InstallSignalQueue} API.
...
\begin{itemize}
    ...
    \item \lstinline{isUrgent} : If \lstinline{qTrue}, the signal will be sent to the front of the signal-queue.
\end{itemize}

\subsubsection*{Return Value}
\lstinline{qTrue} if the provided signal was successfully delivered, otherwise return \lstinline{qFalse}.
...
\begin{tcolorbox}
\HandRight When configuring signal queue with \lstinline{qQueue_Setup}, remember to size it based on the type \lstinline{qSM_Signal_t}.
\end{tcolorbox}
